...
def getFamilyEmails(familyId):
    ...
    response = requests.get(familyContactsURL, headers={'Authorization': 'Bearer ' + token })
    ...
    while (i < len(familyContactsDict)):
        ...
        if ("Father" == familyMemberRecord["Relation"]):
            if (familyMemberRecord["Email"].strip() and "@" in familyMemberRecord["Email"]):
            ...
            if (familyMemberRecord["Email"].strip() and "@" in familyMemberRecord["Email"]):
                if (primaryEmail.strip()):
                   ...
                   primaryEmail = familyMemberRecord["Email"]
        elif ("Grandmother" == familyMemberRecord["Relation"] or "Grandfather" == familyMemberRecord["Relation"]):
            if (familyMemberRecord["Email"].strip() and "@" in familyMemberRecord["Email"]):
                if (primaryEmail.strip()):
...
while (i < len(classesDict["Period"])):
    ...
    response = requests.get(classInfoUrl, headers={'Authorization': 'Bearer ' + token })
    ...
    if len(classStudentsInfo) > 0:	
        ...
        while(j < len(classStudentsInfoDict)):
            ...
            codeTokens = classStudentsInfoDict[j]["Code"].split("-")
            familyCode = codeTokens[0]
            if ("Massachusetts" == familyDict.get(familyCode)["State"]):
            ...
            CityStateZip = '"'+familyDict.get(familyCode)["City"] + "," + familyDict.get(familyCode)["State"].upper() + familyDict.get(familyCode)["ZIP"]+'"'
            print("{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}".format(classStudentsInfoDict[j]["LastName"].strip(),",",classStudentsInfoDict[j]["FirstName"].strip(),",",aClassRecord["Name"],",",aClassRecord["Section"],",",teacherLastName,",",teacherFirstName,",",familyCode,",",'"',familyDict.get(familyCode)["Name"],'"',",",familyDict.get(familyCode)["primaryEmail"],",",familyDict.get(familyCode)["secondaryEmail"],",",familyDict.get(familyCode)["tertiaryEmail"],",",'"',familyDict.get(familyCode)["Address"],'"',",",CityStateZip))
